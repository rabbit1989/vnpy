...
class ChannelBreakVolStrategy(CtaTemplate):
    '''
        一个简单的通道突破波动率结合长假的策略, 这个策略意义不大，回测不行
    '''
    author = "white"
    ...
    def buy_volatility(self, bar):
        ...
        if abs(vol) > 0.0000001:
            ...
            print('111111111111 vol is too small')
    ...
    def on_bar(self, bar):
        ...
        if isinstance(bar, BarData) is True:
            ...
            pre_max_vol = None
            pre_min_vol = None
            self.should_buy = self.should_sell = False
            # 判断突破信号
            if self.am_spot.inited is True:
                pre_max_vol = np.max(self.am_spot.vol_array)
                pre_min_vol = np.min(self.am_spot.vol_array)
            self.am_spot.update_bar(bar)
            if pre_max_vol and pre_min_vol:
                cur_max_vol = np.max(self.am_spot.vol_array)
                cur_min_vol = np.min(self.am_spot.vol_array) 
            ...
            if option_symbol_list:
                ...
                exp_date = datetime.datetime.strptime(full_option_data['delist_date'], '%Y%m%d')
                vol = self.am_spot.vol_array[-1]
                option_price = full_option_data['settle']
                option = Option(full_option_data['call_put'], 
                                self.spot_close_price,
                                full_option_data['strike_price'],
                                bar.datetime.replace(tzinfo=None),
                ...
                option_params = {
                    'delta': myround(delta,3),
                    'vol': myround(vol,3),
                    'imp_vol': myround(imp_vol,3),
                    'theta': myround(theta,4),
                    'gamma': myround(gamma,3),
                    'calc_price': myround(calc_price,3),
                    'option_price': myround(option_price, 3),
                    't': myround(option.t, 3),
                    'vega': myround(vega,3),
                    'pos': myround(self.pos_dict[option_symbol],3),
                    'exp_date': full_option_data['delist_date'],
                    'k': full_option_data['strike_price']
                }

            # 打印当天的波动率
            print('{},spot: {:.3f}, params: {}, return_array: {}'.format(
                bar.datetime.strftime('%F'), 
                self.spot_close_price,
                option_params,
                self.am_spot.return_array),
                )
            
            
            
            # 如果长假快到了且空仓就买入
            #if no_trade_in_days(bar.datetime, 2, 6) and not self.get_option_list():
            #    self.buy_volatility(bar)
            #else:
            if self.should_sell or self.should_buy:
                # 突破最近高点且空仓则买入，低于最近低点且持仓则卖出 
                option_symbol_list = self.get_option_list()
                if self.should_sell and option_symbol_list:
                    # 如果持仓就立刻卖了， 反正是T+1
                    option_symbol = option_symbol_list[0]
                ...
                elif self.should_buy and not option_symbol_list:
                    self.buy_volatility(bar)
                self.should_buy = self.should_sell = False
